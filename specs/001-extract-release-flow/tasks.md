...
- [ ] T010 [US1] Run `bash scripts/derive-version.test.sh`; make B1–B15 + P3–P5 green — fixing only parameterization wiring in `scripts/derive-version.sh`, never the algorithm (any intended deviation documented in [research.md](./research.md) per Constitution VII)
...
- [ ] T011 [P] [US2] Write `scripts/resolve-env.test.sh` covering P1 (`main` → `is-env=true`) and P2 (`feature-x` → `is-env=false`) against the fixture manifest (SC-002)

### Implementation for User Story 2

- [ ] T012 [US2] Run `bash scripts/resolve-env.test.sh`; make P1–P2 green — fixing only `scripts/resolve-env.sh` wiring, preserving the exact source predicate
...
- [ ] T013 [P] [US3] Port `snackbyte-base/scripts/add-env.test.sh` → `scripts/add-env.test.sh`, adjusting paths; assert the diff-guard (only `environments.json` changed), `-qa` derivation, and the resolve-env recognize/reject checks (SC-003)

### Implementation for User Story 3

- [ ] T014 [US3] Run `bash scripts/add-env.test.sh`; make it green (no script change expected — this proves the existing scripts already honor a one-row edit; if a change is needed, that itself is a finding)
...
- [ ] T016 [US4] Author `action.yml` at repo root: composite Action (`runs.using: composite`) with inputs `branch` (default `${{ github.ref_name }}`), `manifest` (default `./environments.json`), `major-minor` (default: read `package.json`) and outputs `is-env`, `version`, `tag`, wiring steps to `scripts/resolve-env.sh` then `scripts/derive-version.sh` per [contracts/action-io.md](./contracts/action-io.md) (research D4)
- [ ] T017 [US4] Run `bash scripts/action.test.sh`; make I1–I2 green — fixing only `action.yml` wiring (input pass-through, output mapping, gating so a non-env push skips derivation)
...
- [ ] T018 Wire `package.json` `test:release` to chain all suites: `derive-version.test.sh` (B1–B15 + P3–P5), `resolve-env.test.sh` (P1–P2), `add-env.test.sh`, `action.test.sh` (I1–I2)
